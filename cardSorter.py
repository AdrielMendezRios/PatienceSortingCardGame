...
class CardSorter():
    ...
    def play(self):
        ...
        The Algorithm:
         ...
         1.Construct the Card piles on the table, calling the
           construct_piles_subseqs method, which also
        ...
        while not self.__deck.is_empty():
            ...
            for pile in self.__piles:
                if not placed_in_existing:
                    card_ref = pile.get_top_card()
                    if card_ref.compare(card_from_deck) == 1:
                        placed_in_existing = True
                        if self.__piles.index(pile) != 0:
                            prev_index = self.__piles.index(pile) - 1
                            prev_pile = self.__piles[prev_index]
                            prev_pile_top = prev_pile.get_top_card()
                            self.add_card_to_subseq(card_from_deck, prev_pile_top)
                        else:
                            self.add_card_to_subseq(card_from_deck, None)
                        pile.add_card(card_from_deck)
            if not placed_in_existing:
                new_pile = Pile()
                if len(self.__piles) == 0:
                    self.add_card_to_subseq(card_from_deck, None)
                elif len(self.__piles) > 0:
                    this_top = self.__piles[-1].get_top_card()
                    self.add_card_to_subseq(card_from_deck, this_top)
                new_pile.add_card(card_from_deck)
                self.__piles.append(new_pile)
    ...
    def find_subseq(self, prev_card):
        ...
        for seq in self.__subseqs:
            if seq.contains_card(prev_card):
                return seq
        return None
    ...
    def display_longest_subseq(self):
        ...
        for seq in self.__subseqs:
            if seq.get_size() == len(self.__piles):
                clone = seq.clone_subseq()
                clone.reverse_subseq()
                print(clone)
        ...
        for i in range(53):
            ...
            for p in range(len(piles)):
                ...
                if not pile.is_empty():
                    first_card = pile.get_top_card()
                    if first_card.compare(low_card) == -1:
                        low_card = first_card
                        low_pile = piles.index(pile)
            card = piles[low_pile].remove_card()
